...
# ============ ADMIN PANEL (YANGILANGAN VA TOZALANGAN) ============

def admin_menu_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Statistika", callback_data="admin_stats")],
        [InlineKeyboardButton(text="📢 Kanallarni boshqarish", callback_data="admin_listchannels")],
        [InlineKeyboardButton(text="💎 VIP berish", callback_data="admin_vip_help")],
        [InlineKeyboardButton(text="🚫 Bloklash/Unban", callback_data="admin_ban_help")],
...
async def admin_panel(message: Message):
    await message.answer("🛠 <b>Admin boshqaruv paneli:</b>", reply_markup=admin_menu_kb(), parse_mode="HTML")
...
@admin_only
async def admin_vip_help(callback: CallbackQuery):
    await callback.message.answer("💎 <b>VIP berish:</b>\n/setvip [user_id] [kunlar]\nMisol: /setvip 7060319031 30")

@router.callback_query(F.data == "admin_ban_help")
@admin_only
async def admin_ban_help(callback: CallbackQuery):
    await callback.message.answer("🚫 <b>Ban/Unban:</b>\n/ban [user_id]\n/unban [user_id]")

# ============ Yagona va to'g'ri funksiyalar ============
...
async def cmd_addchannel(message: Message, bot: Bot):
    ...
    if len(parts) != 2:
        ...
        return
    db.add_required_channel(parts[1].strip())
    await message.answer(f"✅ {parts[1]} qo'shildi.")
...
async def cmd_removechannel(message: Message):
    parts = message.text.split(maxsplit=1)
    if len(parts) != 2:
        await message.answer("Foydalanish: /removechannel [username]")
        return
    db.remove_required_channel(parts[1].strip())
    await message.answer(f"✅ {parts[1]} olib tashlandi.")
